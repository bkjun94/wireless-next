...
# Auto-detect script path based on this module's location
MESH_SCRIPT_PATH = os.path.abspath(__file__)
MESH_SCRIPT_DIR = os.path.dirname(MESH_SCRIPT_PATH)
NRC_PKG_BASE = MESH_SCRIPT_DIR  # This points to nrc_pkg/script

def check(interface):
    """Check if interface has IP address assigned (no dependency on start.py)"""
    ifconfig_cmd = "ifconfig " + interface
    ifconfig_process = subprocess.Popen(ifconfig_cmd.split(),
...
def run_mp(interface, country, security, debug, peermac, ip, batman):
    ...
    if security == 'OPEN':
        if peermac != 0:
            os.system("sed -i " + '"s/#no_auto_peer=1/no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_open.conf ')
        else:
            os.system("sed -i " + '"s/ no_auto_peer=1/ #no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_open.conf ')
        os.system("sudo wpa_supplicant -i" + interface + " -c " + NRC_PKG_BASE + "/conf/" + country + "/mp_halow_open.conf " + debug + " &")
    elif security == 'WPA3-SAE':
        if peermac != 0:
            os.system("sed -i " + '"s/#no_auto_peer=1/no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_sae.conf ')
        else:
            os.system("sed -i " + '"s/ no_auto_peer=1/ #no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_sae.conf ')
        os.system("sudo wpa_supplicant -i" + interface + " -c " + NRC_PKG_BASE + "/conf/" + country + "/mp_halow_sae.conf " + debug + " &")
...
def run_map(wlan, mesh, country, security, debug, peermac, ip, batman):
    ...
    if security == 'OPEN':
        if peermac != 0:
            ...
            os.system("sed -i " + '"s/ no_auto_peer=1/ #no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_open.conf ')
        os.system("sudo wpa_supplicant -i" + mesh + " -c " + NRC_PKG_BASE + "/conf/" + country + "/mp_halow_open.conf " + debug + " &")
    else:
        if peermac != 0:
            os.system("sed -i " + '"s/#no_auto_peer=1/no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_sae.conf ')
        else:
            os.system("sed -i " + '"s/ no_auto_peer=1/ #no_auto_peer=1/g"  " + NRC_PKG_BASE + "/conf/' + country + '/mp_halow_sae.conf ')
        os.system("sudo wpa_supplicant -i" + mesh + " -c " + NRC_PKG_BASE + "/conf/" + country + "/mp_halow_sae.conf " + debug + " &")
    checkMeshJoin(mesh)
    ...
    if security == 'OPEN':
        os.system("sed -i " + '"4s/.*/interface=' + wlan + '/g"  " + NRC_PKG_BASE + "/conf/' + country + '/map_halow_open.conf ')
        os.system("sudo hostapd " + NRC_PKG_BASE + "/conf/" + country + "/map_halow_open.conf " + debug +" &")
    elif security == 'WPA2-PSK':
        os.system("sed -i " + '"4s/.*/interface=' + wlan + '/g"  " + NRC_PKG_BASE + "/conf/' + country + '/map_halow_wpa2.conf ')
        os.system("sudo hostapd " + NRC_PKG_BASE + "/conf/" + country + "/map_halow_wpa2.conf " + debug + "  &")
    elif security == 'WPA3-OWE':
        os.system("sed -i " + '"4s/.*/interface=' + wlan + '/g"  " + NRC_PKG_BASE + "/conf/' + country + '/map_halow_owe.conf ')
        os.system("sudo hostapd " + NRC_PKG_BASE + "/conf/" + country + "/map_halow_owe.conf " + debug + "  &")
    elif security == 'WPA3-SAE':
        os.system("sed -i " + '"4s/.*/interface=' + wlan + '/g"  " + NRC_PKG_BASE + "/conf/' + country + '/map_halow_sae.conf ')
        os.system("sudo hostapd " + NRC_PKG_BASE + "/conf/" + country + "/map_halow_sae.conf " + debug + "  &")
    elif security == 'WPA-PBC':
        os.system("sed -i " + '"4s/.*/interface=' + wlan + '/g"  " + NRC_PKG_BASE + "/conf/' + country + '/map_halow_pbc.conf ')
        os.system("sudo hostapd " + NRC_PKG_BASE + "/conf/" + country + "/map_halow_pbc.conf " + debug + "  &")
        time.sleep(1)
