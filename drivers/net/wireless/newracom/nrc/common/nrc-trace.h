 ...
 *
 * NRC Trace Definitions - Kernel tracepoint support
 */
...
TRACE_EVENT(nrc_test, TP_PROTO(u32 time), TP_ARGS(time),
	    TP_STRUCT__entry(__field(u32, time)),
	    TP_fast_assign(__entry->time = time;),
	    TP_printk("EVT_LOGT:%010u", __entry->time));

DECLARE_EVENT_CLASS(
	nrc_hdr_event,
	TP_PROTO(struct nrc *nw, const void *data, size_t len, int8_t data1,
		 int8_t data2),

	TP_ARGS(nw, data, len, data1, data2),

	TP_STRUCT__entry(
		__string(device, dev_name(nw->dev)) __string(
			driver, dev_driver_string(nw->dev)) __field(u16, fc_etc)
			__field(u8, fc_type) __field(u8, fc_subtype) __array(
				char, addr1,
				ETH_ALEN) __array(char, addr2,
						  ETH_ALEN) __array(char, addr3,
								    ETH_ALEN)
				__field(u16, sn) __field(u8, tid) __field(
					size_t, len) __field(s8, data1)
					__field(s8, data2) __dynamic_array(
						u8, data,
	...
	TP_fast_assign(
		...
		__assign_str(device); __assign_str(driver);
#else
...
#else
		__entry->tid = *ieee80211_get_qos_ctl(hdr) &
			       IEEE80211_QOS_CTL_TID_MASK;
#endif
		__entry->len = nrc_frm_hdr_len(data, len);
		__entry->data1 = data1; __entry->data2 = data2;
	...
	TP_printk(
		...
		//__get_str(device),
		__entry->len,
		(__entry->fc_type == cpu_to_le16(IEEE80211_FTYPE_DATA)) ?
			__print_flags(__entry->fc_subtype, " ",
		...
		(__entry->fc_type == cpu_to_le16(IEEE80211_FTYPE_MGMT)) ?
			get_mgmt_str(__entry->fc_subtype) :
			"???",
		...
		__entry->addr1, __entry->addr2, __entry->sn, __entry->tid,
		__entry->data1, __entry->data2
...
DECLARE_EVENT_CLASS(
	...
	TP_STRUCT__entry(__string(device, dev_name(nw->dev)) __string(
		driver, dev_driver_string(nw->dev)) __field(size_t, len)
				 __dynamic_array(u8, payload,
						 (len -
						  nrc_frm_hdr_len(data, len)))),
	...
	TP_fast_assign(
		...
		memcpy(__get_dynamic_array(payload),
		       data + nrc_frm_hdr_len(data, len),
		       min_t(size_t, __entry->len, MAX_PAYLOAD));),

#if defined __print_hex_dump
	TP_printk("len %zu \n%s%s\n", __entry->len,
		  __print_hex_dump("\t\t\t", DUMP_PREFIX_NONE, 16, 1,
				   __get_dynamic_array(payload),
				   min_t(size_t, __entry->len, MAX_PAYLOAD),
				   false),
	...
	TP_printk("len %zu \n%s%s\n", __entry->len,
		  __print_array(__get_dynamic_array(payload),
				min_t(size_t, __entry->len, MAX_PAYLOAD), 1),
		  (__entry->len > MAX_PAYLOAD) ? "\t\t\t..." : "")
#endif
);
...
DECLARE_EVENT_CLASS(
	...
	TP_STRUCT__entry(__string(msg, msg) __field(u8, dir) __field(u16, head)
				 __field(u16, tail) __field(u16, num)),
...
#endif
		__entry->dir = dir; __entry->head = priv->slot[dir].head;
		__entry->tail = priv->slot[dir].tail;
		__entry->num = priv->slot[dir].head - priv->slot[dir].tail;),

	TP_printk("SLOT[%s] %2hu (%4hu,%4hu) '%s'\n",
		  (__entry->dir == 0) ? "TX" : "RX", __entry->num,
		  __entry->head, __entry->tail, __get_str(msg)));
