...
#define WIM_HEAD_SIZE (sizeof(struct hif_hdr) + sizeof(struct wim_hdr))
#define WIM_MAX_DATA_SIZE (512)
#define WIM_MAX_TLV_SIZE (2048)
#define WIM_MAX_SIZE (WIM_HEAD_SIZE + WIM_MAX_DATA_SIZE + WIM_MAX_TLV_SIZE)
...
enum WIM_CMD_ID {
	...
	WIM_CMD_SCHED_SCAN_START,
	WIM_CMD_SCHED_SCAN_STOP,
	WIM_CMD_MCP_CHAN_ID_CONTROL_H2F,
	WIM_CMD_MCP_CHAN_ID_CONTROL_F2H,
	WIM_CMD_MAX,
};
...
enum WIM_EVENT_ID {
	...
	WIM_EVENT_SCHED_SCAN_COMPLETED,
	WIM_EVENT_MCP_NEXT_GROUP,
	WIM_EVENT_MCP_KEEP_ALIVE,
	WIM_EVENT_MCP_SCHEDULE_REPORT,
	WIM_EVENT_MCP_IMMEDIATE_REPORT,
	WIM_EVENT_MAX,
};
...
enum WIM_TLV_ID {
	...
	WIM_TLV_APF_VERSION,
	WIM_TLV_APF_MAXLEN,
	WIM_TLV_APF_SET_FILTER,
	WIM_TLV_APF_GET_FILTER,
	WIM_TLV_SCHED_SCAN_PARAM,
	WIM_TLV_FW_GET_INFO,
	WIM_TLV_MAX,
};
...
enum { WIM_S1G = 0, WIM_S1G_DUP_1M, WIM_S1G_DUP_2M };
...
#define WIM_MAX_MATCH_SET WIM_MAX_SCAN_SSID
#define WIM_MAX_SCAN_PLAN (3)
...
struct wim_drv_info_param {
	...
	uint8_t band_sel_gpio_num;
	bool band_sel_gpio_polarity;
} __packed;
...
struct wim_scan_match {
	struct wim_scan_ssid ssid;
	struct wim_scan_bssid bssid;
	int8_t rssi_thold;
} __packed;

struct wim_scan_plan {
	uint32_t interval;
	uint32_t iterations;
} __packed;

struct wim_sched_scan_param {
	int8_t min_rssi_thold;
	uint32_t delay;
	uint8_t n_match_sets;
	uint8_t n_scan_plans;
	struct wim_scan_match match_sets[WIM_MAX_MATCH_SET];
	struct wim_scan_plan scan_plans[WIM_MAX_SCAN_PLAN];
} __packed;
...
/* WIM Core Structures (moved from hal/nrc_core/wim.h) */
struct wim {
	union {
		u16 cmd;
		u16 resp;
...
/* wim_credit_report is already defined by WIM_DECLARE macro above */
