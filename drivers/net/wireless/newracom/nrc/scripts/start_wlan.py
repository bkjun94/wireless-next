...
def usage_print():
    print("Usage: \n\tstart_modular.py [sta_type] [security_mode] [country] [channel] [sniffer_mode] \
            \n\tstart_modular.py [sta_type] [security_mode] [country] [mesh_mode] [mesh_peering] [mesh_ip]")
...
def copyConf():
    """Copy configuration files and firmware"""
    copyFirmware()
...
def setWLANModuleParam():
    ...
    fw_arg = power_save_arg = sleep_duration_arg = idle_mode_arg = \
    ...
    wlan_module_param = fw_arg + bd_name_arg + \
...
def run_common():
    ...
    # --- Smart Reload Logic: Get current mode before any cleanup ---
    new_op_mode = strSTA()
    current_op_mode = None
    
    print("[*] Checking current operating mode using 'iw' (before cleanup)...")
    try:
        # Check wlan0 info before any cleanup commands modify its state
        result = subprocess.check_output("iw dev wlan0 info", shell=True, stderr=subprocess.DEVNULL).decode()
        if 'type AP' in result:
            current_op_mode = 'AP'
        elif 'type managed' in result:
            # 'managed' is generic, so we map it based on the new_op_mode for comparison
            # We assume if iw shows 'managed' and we're not aiming for AP/SNIFFER, it's not a fundamental mode switch.
            if new_op_mode == 'AP': # If we want AP, and it's currently managed (STA/Relay/Mesh), it's a switch
                current_op_mode = 'STA' # Generic STA type for comparison
            elif new_op_mode == 'SNIFFER':
                current_op_mode = 'STA' # If target is SNIFFER, and current is managed, treat as STA for switch detection
            else: # new_op_mode is STA, RELAY, MESH
                current_op_mode = new_op_mode # Assume the current type aligns with target if not AP/SNIFFER
        elif 'type monitor' in result:
            current_op_mode = 'SNIFFER'
        print("[*] Current mode detected: %s" % current_op_mode)
    except subprocess.CalledProcessError:
        print("[*] No active wlan0 interface found. Assuming modules are down.")
        current_op_mode = None
    
    # Check if nrc_mcp is loaded. This also needs to happen BEFORE modules are unloaded.
    mcp_loaded = False
    try:
        subprocess.check_output("lsmod | grep -w nrc_mcp", shell=True, stderr=subprocess.DEVNULL)
        mcp_loaded = True
        print("[*] nrc_mcp module detected.")
    except subprocess.CalledProcessError:
        pass # mcp not loaded
    ...
    # Determine if the basic mode type (AP vs STA vs SNIFFER) is changing.
    is_mode_switch = False
    if current_op_mode is None:
        is_mode_switch = True # Modules are down, must do a full load.
    elif new_op_mode == 'AP' and current_op_mode != 'AP':
        is_mode_switch = True
    elif new_op_mode != 'AP' and current_op_mode == 'AP': # Handles STA, MESH, RELAY, SNIFFER when coming from AP
        is_mode_switch = True
    elif new_op_mode == 'SNIFFER' and current_op_mode != 'SNIFFER':
        is_mode_switch = True
    elif new_op_mode != 'SNIFFER' and current_op_mode == 'SNIFFER': # Handles AP, STA, MESH, RELAY when coming from SNIFFER
        is_mode_switch = True
    # For other cases (STA->MESH, MESH->RELAY etc. that iw might show as 'managed' or equivalent),
    # we assume efficient reload. If this proves problematic, we can expand 'is_mode_switch'.
    
    if is_mode_switch:
        print("[*] Mode switch detected (%s -> %s) or modules are down. Performing full reload." % (current_op_mode, new_op_mode))
        # Unload all modules in correct dependency order
        os.system("sudo rmmod nrc_wlan 2>/dev/null")
        if mcp_loaded: # Use the already determined mcp_loaded status
            print("[*] Unloading nrc_mcp module.")
            os.system("sudo rmmod nrc_mcp 2>/dev/null")
        os.system("sudo rmmod nrc_core 2>/dev/null")
        os.system("sudo rmmod nrc_spi 2>/dev/null")
    else:
        print("[*] New mode (%s) is same as current mode. Performing efficient WLAN-only reload." % new_op_mode)
        os.system("sudo rmmod nrc_wlan 2>/dev/null")
    
    os.system("sudo rmmod nrc 2>/dev/null") # Also remove legacy driver if it exists
    # --- End of Smart Reload Logic ---
    ...
    # Only load SPI and CORE if they were unloaded (mode switch or full reload)
    if is_mode_switch:
        ...
        core_insmod_cmd = "sudo insmod /home/pi/nrc_pkg/sw/driver/nrc_core.ko " + core_param
        ...
        print("[3.1-3.2] Skipping SPI and CORE reload (same mode - modules already loaded)")

    # Always reload WLAN frontend module (nrc_wlan.ko)
    print("[3.3] Loading WLAN frontend module (nrc_wlan.ko)")
    wlan_insmod_cmd = "sudo insmod /home/pi/nrc_pkg/sw/driver/nrc_wlan.ko " + wlan_param
    ...
    if not country_set_success:
        ...
        cli_output = subprocess.run(
            ["/home/pi/nrc_pkg/script/cli_app", "test", "country", country_code],
            capture_output=True,
            text=True
        )
