...
discover_peers() {
    ...
    local peers=$(ip neigh show dev $INTERFACE | awk '$1!="'$my_ip'" && $NF!="FAILED" {print $1}' | sort -u)
...
check_port() {
    local peer="$1"
    local port="$2"
    timeout 3 nc -z "$peer" "$port" 2>/dev/null
}
...
run_diagnostics() {
    ...
    log_info "Network diagnostics for $peer"
    
    echo "Ping test:"
    ping -c 5 "$peer" | tail -1
