...
enum { US = 0, JP, K0, K1, TW, EU, CN, NZ, AU, K2, MAX_COUNTRY_CODE };
...
const struct s1g_channel_table *s1g_ch_table_set[MAX_COUNTRY_CODE] = {
	s1g_ch_table_jp, s1g_ch_table_k1_usn, s1g_ch_table_tw,
	s1g_ch_table_us, s1g_ch_table_eu,     s1g_ch_table_cn,
	s1g_ch_table_nz, s1g_ch_table_au,     s1g_ch_table_k2_mic,
};
...
void nrc_set_s1g_country(char *country_code)
{
	...
	if (cc_index >= MAX_COUNTRY_CODE) {
		...
		s1g_alpha2[0] = 'U';
		s1g_alpha2[1] = 'S';
		DBG_MAC("%s: Country index %d out of range. Setting default country (US)!",
			__func__, cc_index);
	}
	if (cc_index < 0) {
		cc_index = US;
		s1g_alpha2[0] = 'U';
		s1g_alpha2[1] = 'S';
		// DBG_MAC("%s: No country code %s. Setting default country (US)", __func__, country_code);
	} else if (cc_index == EU) {
	...
	DBG_MAC("%s: Country code %s, number of channels: %d", __func__,
		country_code, nrc_get_num_channels_by_current_country());
}

const struct s1g_channel_table *nrc_get_current_s1g_cc_table(void)
{
	return &s1g_ch_table_set[nrc_get_current_ccid_by_country(
		nrc_get_current_s1g_country())][0];
}
